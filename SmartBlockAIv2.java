...
/** SmartBlockAIv2
 * 
 * UPDATE v2: Now with strategy randomization!
 * 
...
public class SmartBlockAIv2 extends AI {
	...
	private ArrayList<Integer> openMoves;
	private ArrayList<Integer> enemyMoves;
	private char enemySide;

	//No parameter constructor, initialize name
	public SmartBlockAIv2(){
		this.name = "SmartBlockAIv2";
		stratList = new int[8][3];
		Random ran = new Random();
		ArrayList<Integer> stratInd = new ArrayList<Integer>();
		for (int i = 0; i < 8; i++)
			stratInd.add(i);
		for (int j = 0; j < 8; j++)			// strategy shuffling mechanism
		{
			int nextStrat = ran.nextInt(stratInd.size());
			int stratPick = stratInd.remove(nextStrat);
			System.out.println(stratPick);
			stratList[j] = strategies[stratPick];
		}
	}
	@Override
	public Point move(TTTBoard b) {
		...
		{
			int[] stratCount = new int[stratList.length];
			for (int i = 0; i < myMoves.size(); i++)				// look for strats containing my pieces
			{
				...
				{
					if (prevMove == stratList[j][0] || prevMove == stratList[j][1] || prevMove == stratList[j][2])
					{
						stratCount[j]++;
					}
				}
			...
			int[] stratECount = new int[stratList.length];
			...
			{
				int i = 0;
				System.out.println("Chosen strat: " + strat);
				N = stratList[strat][i];
	...
	{
		if (!enemyMoves.contains(stratList[strat][0])) {
			if (!enemyMoves.contains(stratList[strat][1])) {
				if (!enemyMoves.contains(stratList[strat][2])) {
	...
	{
		if (!myMoves.contains(stratList[strat][0])) {
			if (!myMoves.contains(stratList[strat][1])) {
				if (!myMoves.contains(stratList[strat][2])) {

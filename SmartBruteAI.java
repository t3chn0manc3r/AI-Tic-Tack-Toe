...
/** SmartBruteAI
 * 
 * An improved version of BruteForceAI.
 * After the first move (which is hard-coded), it looks for the next easiest way to win and goes for it.
 * It's still relatively unintelligent, but it actually "thinks" a little.
 * 
...
public class SmartBruteAI extends AI {
...
//DEBUG
	...
	private boolean checkStrat(int strat)
	{
		if (myMoves.contains(strategies[strat][0]) || openMoves.contains(strategies[strat][0])) {
			if (myMoves.contains(strategies[strat][1]) || openMoves.contains(strategies[strat][1])) {
				if (myMoves.contains(strategies[strat][2]) || openMoves.contains(strategies[strat][2])) {
